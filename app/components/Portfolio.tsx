    ...
    <section id="portfolio" className="py-24 px-6 relative overflow-hidden">
      <div className="relative max-w-[1600px] mx-auto">
        <FadeIn className="text-center mb-16">
          ...
          <h2 className="text-4xl sm:text-5xl font-black text-amber-200">{t.portfolio.subline}</h2>
        </FadeIn>
        ...
        {!loading && items.length === 0 && (
          <p className="text-center text-amber-200/40 py-12">Binnenkort meer werk te zien.</p>
        )}
                ...
                >
                  <LiquidCard accentRgb={ACCENT_COLORS[(page * PER_PAGE + i) % ACCENT_COLORS.length]}>
                    ...
                    <div className={`h-48 bg-gradient-to-br ${item.color || "from-violet-900 to-purple-700"} relative overflow-hidden`}>
                      ...
                      <div className="absolute top-0 left-0 right-0 h-7 bg-black/60 backdrop-blur-sm flex items-center px-3 gap-1.5 z-10">
                        ...
                        <div className="ml-2 flex-1 h-3 bg-white/10 rounded-sm flex items-center px-2">
                          <span className="text-amber-200/30 text-[8px] truncate">{item.liveUrl || "example.com"}</span>
                        </div>
                      </div>
                      ...
                      {item.liveUrl ? (
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="w-14 h-14 rounded-2xl bg-white/15 backdrop-blur-sm flex items-center justify-center text-2xl font-black text-amber-200 border border-white/20">
                            {item.title.charAt(0)}
                          </div>
                        </div>
                      )}
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-colors duration-300 flex items-center justify-center z-20">
                        <span className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white/10 backdrop-blur-sm border border-white/20 text-amber-200 text-xs font-bold px-4 py-2 rounded-full">
                          Bekijk website ↗
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-5">
                      ...
                      <h3 className="text-amber-200 font-black text-lg mb-2">{item.title}</h3>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-6 mt-4">
                ...
                <button
                  onClick={prev}
                  disabled={page === 0}
                  className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-amber-200/60 hover:text-amber-200 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
