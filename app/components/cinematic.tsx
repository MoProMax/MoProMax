...
export function Moment({
  eyebrow, title, line, detail, visual, features, flip = false, compact = false, eyebrowClass = "text-amber-400", glow = "251,191,36", check = "#34d399",
}: {
  eyebrow: string;
  title: ReactNode;
  line: string;
    ...
    <div className={`${compact ? "min-h-[70vh]" : "min-h-[84vh]"} flex items-center px-6 py-12`}>
      <div className={`max-w-[1600px] mx-auto w-full grid md:grid-cols-2 gap-10 md:gap-16 items-center ${flip ? "md:[direction:rtl]" : ""}`}>
        <Reveal className="md:[direction:ltr]">
          ...
          <h3 className={`${compact ? "text-4xl sm:text-5xl lg:text-[56px]" : "text-4xl sm:text-5xl lg:text-6xl"} font-black text-amber-400 leading-[1.04] tracking-tight mb-6`}>
            {title}
          </h3>
          {/* core line — shown on every device */}
          <p className="text-lg sm:text-xl text-amber-400/55 leading-relaxed max-w-md">{line}</p>
          {/* extra paragraph — desktop only */}
          {detail && (
            <p className="hidden md:block mt-5 text-lg text-amber-400/45 leading-relaxed max-w-md">{detail}</p>
          )}
          {/* feature checklist — desktop only */}
          {features && features.length > 0 && (
            <ul className="hidden md:block mt-7 space-y-3 max-w-md">
              {features.map((f) => (
                <li key={f} className="flex items-start gap-3 text-amber-400/75 text-base">
...
export function Statement({ eyebrow, children, sub, detail, cta, minH = "min-h-screen", eyebrowClass = "text-amber-400" }: {
  ...
  return (
    <div className={`${minH} flex items-center justify-center px-6 text-center`}>
      <Reveal>
        ...
        <h2 className="text-4xl sm:text-6xl lg:text-7xl font-black text-amber-400 leading-[1.05] tracking-tight max-w-4xl mx-auto">
          {children}
        </h2>
        {sub && <p className="text-amber-400/45 text-lg sm:text-xl mt-8 max-w-xl mx-auto leading-relaxed">{sub}</p>}
        {/* richer prose — desktop only, for readers who take their time */}
        {detail && <p className="hidden md:block text-amber-400/40 text-lg mt-6 max-w-2xl mx-auto leading-relaxed">{detail}</p>}
