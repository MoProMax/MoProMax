    ...
    <div className={`relative inline-flex ${className}`}>
      ...
      <a
        ...
        onMouseEnter={(e) => {
          ...
          el.style.boxShadow =
            "0 0 52px -4px rgba(34,197,94,0.70), 0 28px 52px rgba(0,0,0,0.38)";
        }}
        onMouseLeave={(e) => {
          const el = e.currentTarget as HTMLAnchorElement;
          el.style.transform = "translateY(0) scale(1)";
          el.style.boxShadow =
            "0 6px 24px -4px rgba(34,197,94,0.45), 0 2px 8px rgba(0,0,0,0.18)";
        }}
      >
        ...
        <span
          ...
          style={{
            background:
              "linear-gradient(90deg, transparent, rgba(255,255,255,0.50) 40%, rgba(255,255,255,0.50) 60%, transparent)",
          }}
        />

        {/* Calendar icon */}
        <svg
          className={`${s.icon} flex-shrink-0 relative`}
          fill="none"
          stroke="currentColor"
          strokeWidth={2.2}
          viewBox="0 0 24 24"
        ...
        <span className="relative flex-shrink-0 flex items-center justify-center">
          {/* outer ping */}
          <span
            className={`absolute ${s.dot} rounded-full bg-white/70`}
            style={{ animation: "book-ping 1.8s cubic-bezier(0,0,0.2,1) infinite" }}
          />
          {/* solid core */}
